...
class NotImplimentedError(BaseException):pass
...
class Matrix_Operations:
	...
	def row_reduction(self, pivoit_row, rowB, ratio):
		...
		for x in range(0, len(rowB)):
			reduced_row.append(str(float(float(rowB[x]) - (ratio*float(
															  pivoit_row[x])))))
		return reduced_row
	...
	def mcp(self, matrix):
		...
		copy = self._mm.make_zero_matrix(len(matrix), len(matrix[0]))
		for x in range(0, len(matrix)):
			for y in range(0, len(matrix[0])):
				copy[x][y] = matrix[x][y]
		return copy
	...
	def calculate_eigan_values(self, matrix):
		"""
		Calculates the eigan values of the given matrix

		Precondition:
			matrix must be a 2x2 matrix

		Parameters:
			matrix (Matrix): the matrix for which the eigan values are to be
			calculated
...
class Make_Matrix:
	...
	def make_matrix(self):
		...
		for x in range(0, rows):
			row_values = input("Enter numbers in row {} (eg: 1 2 3) : ".format(
																		   x+1))
...
class Matrix:
	...
	def is_square(self):
		"""(bool) checks if the matrix is square"""
	...
	def size(self):
		"""(str) (nxm) where n is the rows and m is the columns"""
		return "{}x{}".format(len(self._matrix), len(self._matrix[0]))
